...
\small Policy Research Division, School of Public Policy and Energy Systems\\
\small New Delhi -- 110001, India\\
\small \texttt{akshay.sahni@policythesis.org}}
...
\begin{abstract}
India's statutory Renewable Purchase Obligation (RPO) trajectory mandates electricity distribution companies (DISCOMs) and open-access entities to scale their renewable energy procurement from 29.91\% in FY 2024--25 to 43.33\% by FY 2029--30, backed by mandatory financial penalties under Section 14A of the amended Energy Conservation Act 2022. Achieving these ambitious mandates cost-effectively requires moving beyond uncoordinated solar PV build-outs toward dispatchable hybrid wind--solar--battery energy storage systems (BESS). Existing literature relies either on macro policy reviews or simplified 24-hour representative daily models (e.g., HOMER approximations and CEA NEP14 block models) that fail to capture seasonal solar-wind complementarity, multi-day monsoon lulls, and statutory sub-obligation rules. This paper presents an 8,760-hour annual Linear Programming (LP) capacity expansion and operational dispatch model powered by the C++ HiGHS optimization engine. To ensure 100\% data transparency and auditability, solar PV ($\gamma_{\text{pv},t}$) and wind ($\gamma_{\text{wind},t}$) hourly profiles are derived from ECMWF ERA5 reanalysis (covering 5 historical weather years, 2019--2023) and NREL SAM for four key Indian renewable corridors (Rajasthan, Gujarat, Tamil Nadu, Karnataka), while DISCOM demand ($D_t$) is normalized from Grid-India (POSOCO) utility telemetry. Operational BESS health is explicitly incorporated via throughput wear degradation penalties and replacement lifespan sensitivities (7, 10, and 13 years). Evaluating 96 baseline, 144 degradation, 80 multi-year weather, and 48 WACC financial sensitivity scenarios ($r \in \{8\%, 10\%, 12\%\}$), results demonstrate that optimal hybrid portfolios satisfy statutory RPO mandates at landed levelized costs of electricity (LCOE) declining from 4.82--5.58~INR/kWh in FY 2024--25 to 3.58--4.14~INR/kWh by FY 2029--30 under a 4-hour BESS baseline (and 3.47--4.04~INR/kWh under 6-hour optimal storage: 3.47~INR/kWh in Tamil Nadu, 3.66~INR/kWh in Karnataka, 3.80~INR/kWh in Gujarat, and 4.04~INR/kWh in Rajasthan). Concessional green debt ($r = 8\%$) further reduces landed tariffs to 2.96--3.34~INR/kWh under 6-hour storage. Results display strong empirical alignment across western corridors for RTC and storage tenders (Gujarat SECI RTC-I levelized PPA at $+5.6\%$, GUVNL 500~MW BESS standalone capacity charges at $-10.1\%$, and SECI 1,200~MW Peak Power in Rajasthan at $-6.0\%\text{ to }+2.5\%$). Unsubsidized model LCOEs exceed generation-only tariffs by $+48.0\%$ (RUMSL Morena), while high-capacity southern wind resources in TN and KA yield a $24.1\%\text{ to }30.3\%$ cost advantage over commercial FDRE Tranche IV bids due to developer equity margins and strict peak delivery mandates. Inter-annual weather sensitivity confirms minimal LCOE variance across monsoon extremes. A 6-hour BESS configuration represents the global cost-optimal storage duration. Financial mechanics analysis under Section 14A confirms that green PPA procurement is 2.4 to 2.88$\times$ cheaper than statutory REC non-compliance penalties (10.00~INR/kWh), providing DISCOM resource planners with an auditable, bankable roadmap for clean energy transition.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{Data Black Boxes and Synthetic Profiles}: Sizing studies frequently fail to document raw hourly data provenance, relying on unvalidated synthetic profiles or undisclosed weather years. Academic rigor demands full data auditability.
...
To guarantee full scientific transparency and 100\% auditability, all technical parameters and input profiles fed into the 8,760-hour LP optimization engine are documented below:

\begin{enumerate}
    \item \textbf{Multi-Year ECMWF ERA5 Reanalysis Datasets (2019--2023)}: Hourly solar irradiance (GHI, DNI, DHI), 10m/100m wind speeds, and ambient temperatures are sourced from the \textbf{ECMWF ERA5 Global Reanalysis dataset covering 5 historical weather calendar years (2019--2023)}, capturing monsoonal lulls, heavy cloudiness, normal reference years, and El Niño heatwaves. Calendar year 2022 serves as the primary reference baseline for the 96 core capacity expansion scenarios, while the full 2019--2023 multi-year ensemble is evaluated in Section~\ref{sec:weather_sensitivity} to establish inter-annual physical and economic robustness.
    
    \item \textbf{NREL SAM Solar PV Sub-system Assumptions}:
    \begin{itemize}
        \item \textit{Module Tech}: 540~Wp Mono-PERC bifacial PV modules (bifaciality factor = 0.70, temperature coefficient = $-0.35\%/^\circ\text{C}$).
        \item \textit{Mounting \& Tracking}: Single-axis horizontal tracking ($0^\circ$ tilt, $\pm 45^\circ$ rotation limit, backtracking enabled, Ground Coverage Ratio = 0.35).
        \item \textit{System Loss Factors}: Inverter efficiency = 98.5\%, DC-to-AC ratio = 1.25, wiring/soiling/mismatch losses = 6.5\%, annual degradation = 0.5\%/year.
    \end{itemize}

    \item \textbf{NREL SAM Onshore Wind Sub-system Assumptions}:
    \begin{itemize}
        \item \textit{Turbine Model}: 3.3~MW IEC Class IIA wind turbine (hub height = 120~m, rotor diameter = 140~m, cut-in speed = 3.0~m/s, rated speed = 11.5~m/s, cut-out speed = 25.0~m/s).
        \item \textit{Loss Factors}: Wind shear power-law exponent $\alpha = 0.14$, turbine availability = 95.0\%, array wake and environmental losses = 8.5\%.
    \end{itemize}

    \item \textbf{Grid-India Load Telemetry Base Year}: State DISCOM load demand curves ($D_t$) are extracted from actual \textbf{Grid-India (POSOCO) 2022--23 hourly state telemetry data}, normalized to a 1,000~MW peak demand reference system.
...
\subsection{Inter-Annual Weather Robustness and Monsoon Sensitivity (2019--2023)}
\label{sec:weather_sensitivity}

To verify whether our optimized hybrid portfolios maintain RPO compliance and economic stability across inter-annual weather shifts, 80 multi-year sensitivity scenarios were evaluated using ECMWF ERA5 reanalysis data spanning five distinct historical weather years (2019 weak monsoon, 2020 heavy monsoon cloudiness, 2021 normal reference, 2022 monsoon surge baseline, and 2023 El Niño heatwave/drought).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig7_interannual_weather_variability.pdf}
\caption{Landed LCOE inter-annual robustness across 5 historical ECMWF ERA5 weather reanalysis years (2019--2023) for FY 2029--30 4-hour BESS portfolios across four Indian renewable corridors.}
...
\begin{itemize}
    \item \textbf{Rajasthan}: 5-year mean LCOE = 4.12~INR/kWh (range: 3.90--4.40~INR/kWh, baseline 2022 = 4.14~INR/kWh).
    \item \textbf{Gujarat}: 5-year mean LCOE = 3.90~INR/kWh (range: 3.71--4.13~INR/kWh, baseline 2022 = 3.91~INR/kWh).
    \item \textbf{Karnataka}: 5-year mean LCOE = 3.78~INR/kWh (range: 3.60--4.00~INR/kWh, baseline 2022 = 3.78~INR/kWh).
    \item \textbf{Tamil Nadu}: 5-year mean LCOE = 3.60~INR/kWh (range: 3.45--3.78~INR/kWh, baseline 2022 = 3.58~INR/kWh).
\end{itemize}

During extreme drought/heatwave years (such as 2023 El Niño), reduced wind generation during evening hours is buffered by increased daytime solar PV capacity factors and BESS arbitrage, preventing unserved energy loads or statutory RPO shortfalls. In solar-heavy western corridors (Rajasthan and Gujarat), 2020 exhibits slightly higher LCOE than 2019 because monsoonal cloudiness-driven solar PV capacity factor derates dominate over wind generation increases, requiring marginal thermal grid imports during multi-day overcast spells. Overall, this confirms that temporal wind-solar-BESS complementarity provides high physical robustness against climate variability.
...
To ensure 100\% mathematical auditability and computational reproducibility, all 320 LP scenario sweeps (96 baseline, 144 BESS degradation, and 80 multi-year weather instances) were solved using the C++ backend of the open-source HiGHS solver engine \cite{Huangfu2018}. Each 8,760-hour annual dispatch instance comprises $N_{\text{vars}} = 70,085$ continuous decision variables and $N_{\text{constraints}} = 78,843$ linear equality and inequality constraints. 
...
\begin{enumerate}
    ...
    \item \textbf{Climate Shift Projections}: Profile inputs rely on historical ERA5 reanalysis (2019--2023). Integrating CMIP6 climate projection models to capture long-term monsoonal shifts and extreme weather frequency will enhance multi-decadal asset risk assessment.
\end{enumerate}
...
The full optimization model codebase, input data generation scripts, and complete scenario result datasets (CSV format) are publicly available to ensure 100\% scientific reproducibility and auditability. The code is hosted on GitHub at \url{https://github.com/AkshaySahni/r1-india-rpo-bess-optimization} and permanently archived on Zenodo under DOI \url{https://doi.org/10.5281/zenodo.10876000}.
...
\bibitem{MNRE2023}
Ministry of New \& Renewable Energy (MNRE), 2023.
Year End Review 2023: Renewable Energy Progress in India. Press Information Bureau, Government of India, New Delhi.
